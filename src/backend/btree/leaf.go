...
import (
	...
	"fmt"
	"log"
	"my-cstack-db/src/accessories/utils/constants"
	"my-cstack-db/src/accessories/utils/helpers"
)
...
func (ln *LeafNode) serialize() []byte {
	...
	pos = helpers.AdvanceCursor(pos, nodeHeaderSize())
...
func (ln *LeafNode) serializeCells() ([]byte, error) {
	...
	for _, cell := range cells {
		if cell != nil {
			...
			pos = helpers.AdvanceCursor(pos, constants.BTreeKeySize)
			copy(buf[pos:], cell.data)
			pos = helpers.AdvanceCursor(pos, len(cell.data))
		}
	}
...
func (ln *LeafNode) deserializeCells(bytes []byte) error {
	...
	for i := 0; i < int(ln.Header.NumCell); i++ {
		...
		pos = helpers.AdvanceCursor(pos, ln.Header.CellSize)
	}
	return nil
}
